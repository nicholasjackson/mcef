...
public class RemoteConfig {
    ...
    /**
     * Updates the MCEF configuration file and parses it. Fills the resources,
     * extract and version fields from it.
     */
    public void load() {
        ...
        if (!arch.equals("32") && !arch.equals("64")) { // Shouldn't happen.
    ...
    public boolean downloadMissing(IProgressListener ipl) {
        ...
        if (resources.size() > 0) {
            ...
            for (String r : extract) {
                ...
                if (!res.extract(ipl)) // Probably not a huge problem if we can't extract some resources... no need to
                                       // return.
