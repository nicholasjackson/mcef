...
public class ClientProxy extends BaseProxy {
    ...
    public void onPreInit() {
        ...
        exampleMod.onPreInit(); // Do it even if example mod is disabled because it registers the "mod://"
                                // scheme
    }
    ...
    public void onInit() {
        ...
        if (OS.isLinux()) {
            ...
            if (Arrays.stream(libPath).filter(s -> !s.isEmpty()).map(fs::getPath)
                    .noneMatch(p -> Util.isSameFile(p, here))) {
                Log.error(
                        "On Linux, you *HAVE* to add the .minecraft folder to LD_LIBRARY_PATH in order for MCEF to work.");
                Log.error(
                        "You can do this by running the following command and then starting Minecraft within the same terminal:");
                ...
                if (ans == JOptionPane.YES_OPTION) {
                    try {
                        ...
                        JOptionPane.showMessageDialog(null,
                                "Couldn't automatically open the wiki page. The link is:\n" + LINUX_WIKI, "MCEF Linux",
        ...
        File subproc = new File(ROOT, exe);
        if (OS.isLinux() && !subproc.canExecute()) {
            try {
                int retCode = Runtime.getRuntime()
                        .exec(new String[] { "/usr/bin/chmod", "+x", subproc.getAbsolutePath() }).waitFor();

                if (retCode != 0)
                    throw new RuntimeException("chmod exited with code " + retCode);
            } catch (Throwable t) {
                Log.errorEx(
                        "Error while giving execution rights to jcef_helper. MCEF will probably enter virtual mode. You can fix this by chmoding jcef_helper manually.",
                        t);
            }
        }
        ...
        // settings.locales_dir_path = (new File(ROOT,
        // "MCEFLocales")).getAbsolutePath();
        ...
        try {
            ...
            // load the frameworks for mac osx from the standard paths, can provide override
            // as args
            Boolean started = CefApp.startup(new String[] {});
            if (!started) {
                Log.error("Unable to start CefApp");
                VIRTUAL = true;
                return;
            }
    ...
    public void addToClassPath(String dir)
        ...
        newList[0] = dir.replace('/', File.separatorChar);
        pathsField.set(null, newList);

        // also set the java paths
        System.setProperty("java.library.path", String.join(System.getProperty("path.separator"), newList));
    }
    ...
    @Override
    public void registerScheme(String name, Class<? extends IScheme> schemeClass, boolean std, boolean local,
            boolean displayIsolated, boolean secure, boolean corsEnabled, boolean cspBypassing, boolean fetchEnabled) {
        appHandler.registerScheme(name, schemeClass, std, local, displayIsolated, secure, corsEnabled, cspBypassing,
                fetchEnabled);
    }
    ...
    public void loadMimeTypeMapping() {
        ...
        try {
            BufferedReader br = new BufferedReader(
                    new InputStreamReader(ClientProxy.class.getResourceAsStream("/assets/mcef/mime.types")));
